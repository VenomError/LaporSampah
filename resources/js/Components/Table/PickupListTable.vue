...
<template>
  <div class="space-y-6">
    <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
      ...
      <div class="flex flex-wrap items-center gap-3">
        <select v-model="params.status" class="px-4 py-2.5 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold outline-none focus:ring-4 focus:ring-green-500/10 transition-all">
        ...
        <input v-model="params.created_at" type="date" class="px-4 py-2.5 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold outline-none focus:ring-4 focus:ring-green-500/10 transition-all" />
        
        <button @click="reset" class="p-2.5 text-red-500 hover:bg-red-50 rounded-xl transition-colors" title="Reset Filter">
      ...
      >
        ...
        <template #member_name="data">
          <div class="flex flex-col">
              <span class="font-black text-slate-900 leading-none">{{ data.value.member_name }}</span>
              <span class="text-[10px] font-bold text-slate-400 uppercase mt-1">ID Warga: {{ data.value.member?.id || '-' }}</span>
          </div>
        </template>

        <template #operator_name="data">
          <div v-if="data.value.operator_name" class="flex items-center gap-2">
              <div class="w-7 h-7 bg-blue-50 text-blue-600 rounded-lg flex items-center justify-center italic text-[10px] font-black">OP</div>
        ...
        <template #created_at="data">
            <div class="flex flex-col text-[11px] font-bold text-slate-500">
                ...
                <span class="text-[9px] text-slate-300 uppercase leading-none">{{ new Date(data.value.created_at).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' }) }} WIB</span>
            </div>
        </template>

        <template #actions="data">
          <div class="flex gap-2">
            <Link :href="route('dashboard.pickup.list')" class="w-9 h-9 flex items-center justify-center bg-slate-50 text-slate-400 rounded-xl hover:bg-green-600 hover:text-white transition-all">
  ...
  <BtModal title="Tugaskan Petugas" ref="modalAssign" @close="formAssign.reset()">
    <form @submit.prevent="submitOperator" class="space-y-4">
      <div class="p-4 bg-green-50 rounded-2xl border border-green-100 mb-4">
          ...
          <p class="text-sm font-bold text-slate-900 mt-1">Mengatur petugas untuk penjemputan ID #{{ formAssign.pickup_id }}</p>
      </div>
      ...
      <div class="flex justify-end pt-4">
          <button class="px-8 py-3 bg-green-600 text-white rounded-2xl font-black uppercase tracking-widest text-xs shadow-xl shadow-green-100 hover:bg-green-700 transition-all" type="submit">
            Konfirmasi Petugas
          </button>
      </div>
    </form>
  ...
  <BtModalConfirm
    ...
    message="Laporan ini akan ditandai sebagai ditolak dan warga akan mendapatkan notifikasi."
    icon="trash-x"
    icon-color="danger"
    confirm-text="Ya, Tolak Laporan"
