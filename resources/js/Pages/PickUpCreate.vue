...
<template>
  ...
  <form @submit.prevent="submit" class="flex flex-col gap-4 pt-8 px-6">
    <div
      class="mt-8 p-6 border border-bgColor2 rounded-2xl dark:border-darkN40 w-full"
      v-if="locations"
    >
      <p
      ...
      <div
        class="suggested-languages flex flex-col gap-4 pt-5"
      ...
      >
        ...
        >
          ...
          <i
            class="ph-radio-button text-2xl radioButton text-g300"
            :class="location.id == form.location_id ? 'ph-fill' : 'ph'"
          ></i>
        </div>
      </div>
    </div>
