...
<template>
    <div class="flex justify-center items-center text-center flex-col">
        ...
        <p v-if="!$page.props.flash.message" class="tet-sm text-n500 dark:text-darkN500 pt-3 px-4">
            Stay connected, follow teams, and never miss thrilling Stay connected
        </p>
        <p v-if="$page.props.flash.message" class="tet-sm text-danger pt-3 px-4 ">
            {{ $page.props.flash.message }}
        </p>
    </div>

    <Form :action="route('register.process')" method="POST" class="flex flex-col gap-4 pt-8" #default="{
        errors,
        processing,
    }" >
        <Input name="name" :error="errors.name" label="Name" placeholder="Masukkan Nama" type="text" />
        <Input name="phone" :error="errors.phone" label="Phone" placeholder="Masukkan Phone" type="tel" />
