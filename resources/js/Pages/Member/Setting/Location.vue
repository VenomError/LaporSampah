...
<template>
    <HeaderTitle href="profile" title="Lapor Sampah" />
    ...
    <form @submit.prevent="submit" class="flex flex-col gap-4 pt-8 px-6">
        <Input name="name" v-model="form.name" :error="form.errors.name" label="Name"
            :placeholder="loading ? 'Loading...' : 'Simpan Nama Lokasi seperti : Rumah, Kantor'" :disabled="loading" />
        <Input name="latitude" v-model="form.latitude" :error="form.errors.latitude" label="Latitude"
            :placeholder="loading ? 'Loading...' : 'Pilih Lokasi di Map'" :disabled="loading" />

        <Input name="longitude" v-model="form.longitude" :error="form.errors.longitude" label="Longitude"
            :placeholder="loading ? 'Loading...' : 'Pilih Lokasi di Map'" :disabled="loading" />

        <InputTextarea name="address" v-model="form.address" :error="form.errors.address"
            label="Alamat / Deskripsi Lokasi" :placeholder="loading
    ...
    <div class="px-6 pt-8" v-if="locations">
        ...
        <div class="flex flex-col gap-4 pt-6">
            <template v-for="(location, index) in locations" :key="index">
                <a class="flex justify-between items-center border-b border-dashed border-n40 dark:border-darkN40 pb-4">
